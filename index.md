---
layout: home-page
title: Dynamic .NET TWAIN Docs
keywords: Dynamic .NET TWAIN Docs, homepage
description: Dynamic .NET TWAIN Docs homepage
breadcrumbText: Dynamic .NET TWAIN Docs
---

# Dynamic .NET TWAIN Docs

- [ImageCore & Viewer Modules](imagecore-viewer-module/)
    - [How to save selected images as multi-page PDF](imagecore-viewer-module/save-pdf.md)
    - [How to stamp text & icons onto acquired images in Dynamic .NET TWAIN](imagecore-viewer-module/stamp-onto-image.md)
    - [How to zoom images using mouse wheel](imagecore-viewer-module/zoom-by-mousewheel.md)
    - [How to automatically deskew an image](imagecore-viewer-module/deskew-image.md)

- [TWAIN Scan Module](twainscan-module/)
    - [Pre-scan settings](twainscan-module/pre-scan-settings.md)

- [Camera Module](camera-module/)
    - [How to rotate video](camera-module/how-to-rotate-video.md)

- [PDF Module](pdf-module/)
    - [PDF Module FAQ](pdf-module/pdf-module.md)

- [OCR Module](ocr-module/)
    - [How to add other languages support in the OCR Demo](ocr-module/how-to-add-languages.md)


- [Troubleshooting](troubleshooting/)
    - [Troubleshooting](troubleshooting/troubleshooting.md)


- [Upgrade Dynamic .NET TWAIN](upgrade-dotnet-twain/)
    - [How to Upgrade](upgrade-dotnet-twain/how-to-upgrade.md)
    


